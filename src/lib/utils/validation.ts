...
export function isValidConnection(
  ...
  // For LLM node, check specific handles
  if (targetType === 'llmNode') {
    return validTargets.includes(targetHandle);
  }
  
  // For crop image node, needs image_url
  if (targetType === 'cropImageNode' && sourceHandle === 'image_url') {
    return true;
  }
  
  // For extract frame node, needs video_url
  if (targetType === 'extractFrameNode' && sourceHandle === 'video_url') {
    return true;
  }
